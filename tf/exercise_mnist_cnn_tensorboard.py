...
summary:
    ...
    to start tensorboart, then, visit:
        ...
        ps: the --logdir when start tensorflow is the directory where
            tensorflow write data in with:
            tf.summary.FileWriter("/tmp/tensorboard/test")
"""
...
def model(x, y_, w, w2, w3, w4):
    ...
    # does softmax needed? no, because of the cost function
...
# accuracy
predict_accuracy = tf.equal(tf.argmax(y, 1), tf.argmax(y_, 1))
accuracy = tf.reduce_mean(tf.cast(predict_accuracy, "float"))

# tensorboard
tf.summary.scalar('accuracy', accuracy)
merged = tf.summary.merge_all()
summary_writer = tf.summary.FileWriter("/tmp/tensorboard/test")
    ...
    while step < training_times:
        ...
        for start in index_start:
            ...
            if (start / batch_size) % 10 == 0:
                # accuracy on training data sets
        ...
        # it is impossible to pass all of the test data to the model,
        # because the limit of the memory, here randomly choose test data
        # from test_x with the size of batch_size
        test_index = np.arange(len(test_x))
        np.random.shuffle(test_index)  # let test_index randomly sorted
        test_slice = test_index[0: batch_size]  # choose a slice
        # get random data from test data sets
        test_x_batch = test_x[test_slice]
        test_y_batch = test_y[test_slice]

        accuracy_test, _tmp = sess.run([merged, accuracy],
                                       feed_dict={x: test_x_batch,
                                                  y_: test_y_batch,
                                                  dropout: 0.5})
        summary_writer.add_summary(accuracy_test, step)
        print("test accuracy: %s after %d step" % (_tmp, step))
